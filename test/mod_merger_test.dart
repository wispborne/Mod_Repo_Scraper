...
void main() {
  group('extractForumTopicId', () {
    ...
    test('extracts topic ID from http URL', () {
      expect(
        ModMerger.extractForumTopicId(
            'http://fractalsoftworks.com/forum/index.php?topic=15122.0'),
        equals('15122'),
      );
    });
    ...
    test('extracts topic ID from URL without trailing .0', () {
      expect(
        ModMerger.extractForumTopicId(
            'https://fractalsoftworks.com/forum/index.php?topic=25205'),
        equals('25205'),
      );
    });
    ...
    test('extracts topic ID from URL with message anchor', () {
      expect(
        ModMerger.extractForumTopicId(
            'https://fractalsoftworks.com/forum/index.php?topic=27367.msg404012#msg404012'),
        equals('27367'),
      );
    });
    ...
    test('returns null for URL without topic parameter', () {
      expect(
        ModMerger.extractForumTopicId(
            'https://fractalsoftworks.com/forum/index.php'),
        isNull,
      );
    });
  });
    ...
    test('http and https variants of same topic match', () async {
      final mods = [
        ScrapedMod(
          ...
          urls: {
            ModUrlType.Forum:
                'https://fractalsoftworks.com/forum/index.php?topic=15122.0'
          },
    ...
    test('URL with and without trailing .0 for same topic match', () async {
      final mods = [
        ScrapedMod(
          ...
          urls: {
            ModUrlType.Forum:
                'https://fractalsoftworks.com/forum/index.php?topic=25205.0'
          },
          sources: [ModSource.Index],
        ),
        ScrapedMod(
    ...
    test('URL with message anchor matches same topic', () async {
      final mods = [
        ...
        ScrapedMod(
          ...
          urls: {
            ModUrlType.Forum:
                'https://fractalsoftworks.com/forum/index.php?topic=27367.0'
          },
          sources: [ModSource.Discord],
        ),
      ];
    ...
    test('different topic IDs do not match on URL alone', () async {
      final mods = [
        ...
        ScrapedMod(
          ...
          urls: {
            ModUrlType.Forum:
                'https://fractalsoftworks.com/forum/index.php?topic=22222.0'
          },
          sources: [ModSource.Index],
        ),
      ];
  ...
  group('Name length-ratio guard', () {
    ...
    test('Known Skies and Unknown Skies are NOT merged (ratio 0.83)', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(2),
          reason: '"Known Skies" and "Unknown Skies" should be separate mods');
    });
    ...
    test('ApproLight and ApproLight Plus are NOT merged (ratio 0.71)', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(2),
          reason: '"ApproLight" and "ApproLight Plus" should be separate mods');
    });
    ...
    test('minor name typo still merges (ratio > 0.85)', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(1),
          reason: 'Minor name typo should still merge');
    });
    ...
    test('exact same name always merges', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(1),
          reason: 'Exact same name with aliased author should merge');
    });
  });
  ...
  group('Trigram-indexed candidate generation', () {
    ...
    test('mods with similar names are found via trigram index', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(1),
          reason: 'Same mod with version suffix should merge');
    });
    ...
    test('completely unrelated mods stay separate', () async {
      ...
      expect(result.length, equals(3),
          reason: 'Three different libs should stay separate');
    });
    ...
    test('mods with short names (< 3 chars) still handled', () async {
      ...
      final result = await merger.merge(mods);
      expect(result.length, equals(1),
          reason: 'Short-named mods should still merge via exact name bucket');
    });
    ...
    test('large set of distinct mods stays separate', () async {
      ...
      final distinctNames = [
        'GraphicsLib', 'MagicLib', 'LazyLib', 'LunaLib', 'Nexerelin',
        'Interstellar Imperium', 'Tahlan Shipworks', 'Diable Avionics',
        'Underworld', 'SpeedUp', 'Audio Plus', 'FleetBuilder',
        'Hyperdrive', 'Combat Chatter', 'Planet Search', 'Starship Legends',
        'Ruthless Sector', 'Perilous Expanse', 'Nomadic Survival', 'New Beginnings',
      ];
      final distinctAuthors = [
        'DarkRevenant', 'Tartiflette', 'LazyWizard', 'LukasZero', 'Histidine',
        'DarkRevenantTwo', 'NiaTahl', 'CaymonJoestar',
        'DarkRevenantThree', 'DarkRevenantFour', 'DarkRevenantFive', 'SNuman',
        'Sundog', 'HistidineTwo', 'andylizi', 'SundogTwo',
        'SundogThree', 'SundogFour', 'SundogFive', 'SundogSix',
      ];
      final mods = List.generate(20, (i) => ScrapedMod(
      ...
      expect(result.length, equals(20),
          reason: 'All distinct mods should remain separate');
    });
  });
}
