...
Let the command line hand its forum job to a running server, so the work can be watched in the browser while the console looks exactly as it always did — and do the job itself when the server isn't there.
## Requirements
